...
{
    mRecognizers.append(QSharedPointer<AbstractRecognizer>(
                            new AbstractRecognizer(QSharedPointer<GesturesManager>(new RectangleGesturesManager()))));
    mRecognizers.append(QSharedPointer<AbstractRecognizer>(
                            new AbstractRecognizer(QSharedPointer<GesturesManager>(new NearestPosGridGesturesManager()))));
...
void AdaBoostManager::learn(QList<Entity> const & trainingSet)
{
    ...
    foreach (QSharedPointer<AbstractRecognizer> const &recognizer, mRecognizers)
	{
