...
public:
    TestThread(QSharedPointer<RecognizerInterface> const &gesturesManager, QObject * parent) :
			QThread(parent)
	...
	void run()
	{
		...
		foreach (QString object, usersGestures.keys())
		{
			...
			foreach (QString pathStr, usersGestures[object].second)
			{
				...
				allGestures ++;
                QList<QList<QPoint> > path = stringToPath(pathStr);
...
private:
    QSharedPointer<RecognizerInterface> mRecognizer;
