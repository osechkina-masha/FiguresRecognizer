    ...
    , mRecognizer(new AbstractRecognizer(mGesturesManager))
{
    ...
    mPaintManager = QSharedPointer<PaintManager>(new PaintManager(ui->gvGesture));
...
void MouseGestures::openTestGestures()
{
    ...
    mRecognizer.reset(new AbstractRecognizer(mGesturesManager, usersGestures));
	showTable();
}
	...
	foreach (QString const &object, usersGestures.keys()) {
		...
		for (int i = - maxAngle; i <= maxAngle; i += angleDiff) {
			...
			for (int j = - maxSecondAngle; j <= maxSecondAngle; j += angleDiff) {
                ...
                newGestures.push_back(pathToString(gesture));
			}
		}
...
void MouseGestures::loadFile()
{
	...
	//mGesturesManager = new LevenshteinHullGesturesManager();
    mRecognizer.reset(new AbstractRecognizer(mGesturesManager, entities));
